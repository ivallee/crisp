module.exports = (knex) => {
  ...
  const categorizeRecipe = async (id, userID, category_id) => {
    return await knex('saved_recipes').where('id', id).andWhere('user_id', userID).update({ category_id });
  };

  const deleteRecipe = async (id, userID) => {
    return await knex('saved_recipes').where('id', id).andWhere('user_id', userID).del();
  };
